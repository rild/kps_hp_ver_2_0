...
for ($i = 0; $i < $size; $i++) {
  ...
  $result = pg_execute($db_conn, "delete", array($member_id));
  if (!$result) {
    // 実行に失敗した
    $url = $endpoint.$path.'delete.php' ;
    header("Location: {$url}?sidebar=4");
    ...
    $result = pg_execute($db_conn, "log", array($table_key, 'delete the account:'.$member_id , $user_key, $now ));
    // <---------- log終わり
